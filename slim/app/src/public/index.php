...
function connectdb(){
    $db = new Database("demo", "root", "", "localhost"); 
    return  $db;
}
...
//    $db->select("student");  //select query
...
// INSERT DATA
    ...
    $app->post('/insert', function ($request, $response) {
         ...
         $insert=$db->insert( 'student',  array( "firstname"=> $firstname, "lastname" => $lastname,"email"=>$email,"dob"=>$dob,"gender"=>$gender,"hobby"=>$hobby,"education"=>$education,"description"=>$description ,"profile"=>$profile ) );
          if($insert)
          {
...
// DELETE DATA
        ...
        $app->post('/delete', function ($request, $response) {
         ...
         $params = $request->getParsedBody();
          $id=$params['id'];
          $select=$db->select('student',array("id"=>$id));
          $shop_data =$db->result_array();
         // print_r($shop_data);
         ...
         // echo $row[9];
          $delete=$db->delete('student',array("id"=>$id));
         // print_r($delete);
          if($delete)
          {
            unlink('./img/'.$row[9]);
...
// VIEW DATA
        ...
        $app->post('/view',function($request,$response){
          ...
          <table class="table table-bordered">
              <tr>
                ...
                <th class="column_sort" id="lastname ">Lastname <br/><input type="button" class="sort" id="desc" data-name="lastname" value="↓"><input type="button" class="sort" data-name="Lastname" id="asc" value="↑"></th>
                ...
                <th class="column_sort" id="Description ">Description <br/><input type="button" class="sort" id="desc" data-name="description" value="↓"><input type="button" class="sort" data-name="description" id="asc" value="↑"></th>
                <th>Profile</th>
                <th>Action</th>
              </tr>
          ';

          $view=$db->query("SELECT * FROM student ORDER BY $name $sort LIMIT $start_from,$record_per_page");
        ...
        <button type="button" class="btn btn-danger delete" data-id3='. $row[0].' name="delete">Delete</button></td></tr>';
          ...
          $page_result=$db->query("SELECT * FROM student ORDER BY $name $sort");
...
// UPDATE DATA
    ...
    $description=$data->description;
    $profile=$data->firstname.".png";
          $up=$db->update( 'student', array("Firstname"=>$firstname,"Lastname"=>$lastname,"Email"=>$email,"DOB"=>$dob,"Gender"=>$gender,"Hobby"=>$hobby,"Education"=>$education,"Description"=>$description,"profile"=>$profile), array( "id"=> $id) );
          if($up)
          {
            echo "Update";
          }
...
$app->post('/search',function($request,$response){
  ...
  if(isset($params['search_param'])){
    $search_param = mysqli_real_escape_string(mysqli_connect("localhost","root","","demo"),$params['search_param']);
  $db->query("SELECT * FROM student WHERE firstname Like '%$search_param%' OR Lastname Like '%$search_param%' OR Email Like '%$search_param%' OR DOB Like '%$search_param%' OR Gender Like '$search_param%' OR Hobby Like '%$search_param%' OR Education Like '%$search_param%' OR Description Like '%$search_param%' ORDER BY id DESC LIMIT $start_from,$record_per_page");
  $shop_data= $db->result_array();
  foreach($shop_data as $x=>$x_value)
  {
    $row = array_values($x_value);
...
<button type="button" class="btn btn-danger delete" data-id3='. $row[0].' name="delete">Delete</button></td></tr>';
  ...
  $value.='</table><br/><div align="center">';
  $page_result=$db->query("SELECT * FROM student WHERE firstname Like '%$search_param%' OR Lastname Like '%$search_param%' OR Email Like '%$search_param%' OR DOB Like '%$search_param%' OR Gender Like '$search_param%' OR Hobby Like '%$search_param%' OR Education Like '%$search_param%' OR Description Like '%$search_param%' ORDER BY id DESC");
  $total_records = sizeof($db->result_array());
  $total_pages = ceil($total_records/$record_per_page);
  //echo $total_pages;
...
$app->post('/login',function($request,$response){
  ...
  {
    $db->select('student',array("Email"=>$email));
    $shop_data=$db->result_array();
